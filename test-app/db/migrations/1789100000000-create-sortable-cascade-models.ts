...
export async function up(db: Kysely<any>): Promise<void> {
  ...
  await db.schema
    .createTable('sortable_cascade_leaves')
    .addColumn('id', 'bigint', col => col.primaryKey().generatedByDefaultAsIdentity())
    .addColumn('owner_id', 'bigint', col =>
      col.references('sortable_cascade_owners.id').onDelete('cascade').notNull()
    )
  ...
  await DreamMigrationHelpers.addDeferrableUniqueConstraint(db, 'sortable_cascade_leaves_position', {
    table: 'sortable_cascade_leaves',
    columns: ['owner_id', 'position'],
  })
}

export async function down(db: Kysely<any>): Promise<void> {
  await db.schema.dropTable('sortable_cascade_leaves').execute()
