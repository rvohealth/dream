...
export default class MysqlQueryDriver<DreamInstance extends Dream> extends KyselyQueryDriver<DreamInstance> {
  ...
  public static override async saveDream(dream: Dream, txn: DreamTransaction<Dream> | null = null) {
    ...
    const sqlifiedAttributes = normalizeMysqlTemporalStrings(sqlAttributes(dream))
...
function normalizeMysqlTemporalStrings<T extends Record<string, any>>(attrs: T): T {
  return Object.entries(attrs).reduce((result, [key, value]) => {
    result[key as keyof T] = normalizeMysqlTemporalValue(value)
    return result
  }, {} as T)
}

function normalizeMysqlTemporalValue(value: any): any {
  if (Array.isArray(value)) return value.map(normalizeMysqlTemporalValue)
  if (typeof value === 'string') return value.replace(/ Z$/, '')
  return value
}
