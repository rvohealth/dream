...
export default async function destroyAssociatedRecords<I extends Dream>(
  ...
  for (const associationName of dreamClass['dependentDestroyAssociationNames']()) {
    ...
    for (const record of records) {
      ...
      // Which edge carried the cascade to this record is what the sortable
      // phases of its destroy need in order to tell whether the cascade is
      // taking its whole sort scope with it; see `planSortableDestroyWork`.
