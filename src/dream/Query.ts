...
import CannotAssociateThroughPolymorphic from '../exceptions/associations/CannotAssociateThroughPolymorphic'
import CannotJoinPolymorphicBelongsToError from '../exceptions/associations/CannotJoinPolymorphicBelongsToError'
import JoinAttemptedOnMissingAssociation from '../exceptions/associations/JoinAttemptedOnMissingAssociation'
import MissingRequiredAssociationWhereClause from '../exceptions/associations/MissingRequiredAssociationWhereClause'
import MissingRequiredPassthroughForAssociationWhereClause from '../exceptions/associations/MissingRequiredPassthroughForAssociationWhereClause'
import MissingThroughAssociation from '../exceptions/associations/MissingThroughAssociation'
import MissingThroughAssociationSource from '../exceptions/associations/MissingThroughAssociationSource'
import CannotCallUndestroyOnANonSoftDeleteModel from '../exceptions/CannotCallUndestroyOnANonSoftDeleteModel'
import CannotNegateSimilarityClause from '../exceptions/CannotNegateSimilarityClause'
import CannotPassAdditionalFieldsToPluckEachAfterCallback from '../exceptions/CannotPassAdditionalFieldsToPluckEachAfterCallback'
import CannotPassUndefinedAsAValueToAWhereClause from '../exceptions/CannotPassUndefinedAsAValueToAWhereClause'
import MissingRequiredCallbackFunctionToPluckEach from '../exceptions/MissingRequiredCallbackFunctionToPluckEach'
import NoUpdateAllOnJoins from '../exceptions/NoUpdateAllOnJoins'
import NoUpdateOnAssociationQuery from '../exceptions/NoUpdateOnAssociationQuery'
...
export default class Query<DreamInstance extends Dream> extends ConnectedToDB<DreamInstance> {
  ...
  >(...args: [...Arr, VariadicJoinsArgs<DB, Schema, TableName, Arr>]) {
    ...
    return this.clone({ innerJoinDreamClasses, innerJoinStatements, innerJoinWhereStatements })
  }
  ...
  >(...args: [...Arr, VariadicJoinsArgs<DB, Schema, TableName, Arr>]) {
    ...
    return this.clone({ innerJoinDreamClasses, leftJoinStatements, leftJoinWhereStatements })
  }
  ...
  >(...args: [...Arr, VariadicPluckThroughArgs<DB, Schema, TableName, Arr>]): Promise<any[]> {
    ...
    const pluckStatement = [
      this.fleshOutPluckThroughStatements(
        innerJoinDreamClasses,
        innerJoinStatements,
    ...
    const vals = await this.clone({
      innerJoinDreamClasses,
      innerJoinStatements,
      innerJoinWhereStatements,
    }).executePluck(...pluckStatement)
  ...
  >(...args: [...Arr, VariadicPluckEachThroughArgs<DB, Schema, TableName, Arr>]): Promise<void> {
    ...
    const baseQuery = this.clone({ innerJoinDreamClasses, innerJoinStatements, innerJoinWhereStatements })
  ...
  public async minMaxThrough<DB extends DreamInstance['DB'], Schema extends DreamInstance['schema']>(
    ...
    const query = this.clone({ innerJoinStatements, innerJoinWhereStatements })
    ...
    // eslint-disable-next-line @typescript-eslint/unbound-method
    const { min, max } = query.dbFor('select').fn
  ...
  >(...args: [...Arr, VariadicCountThroughArgs<DB, Schema, TableName, Arr>]): Promise<number> {
    ...
    // eslint-disable-next-line @typescript-eslint/unbound-method
    const { count } = query.dbFor('select').fn
    const countClause = distinctColumn
      ? count(sql`DISTINCT ${distinctColumn}`)
      : count(query.namespaceColumn(query.dreamInstance.primaryKey))
