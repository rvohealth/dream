...
export default class DreamDbConnection {
  ...
  public static async dropAllConnections() {
    for (const connectionName of Object.keys(connections)) {
      await this.dropAllConnectionsForConnectionName(protectAgainstPollutingAssignment(connectionName))
    }
  }

  public static async dropAllConnectionsForConnectionName(connectionName: string) {
    const protectedName = protectAgainstPollutingAssignment(connectionName)
    for (const key of Object.keys(connections[protectedName]!)) {
      await connections[protectedName]![key]?.destroy()
...
export async function closeAllDbConnections() {
  ...
  for (const connectionName of connectionNames) {
    await Promise.all(Object.values(connections[connectionName]!).map(conn => conn.destroy()))
  }
  connections = {}
}
