...
${output}`)
  ...
  private replaceStringWithClockTimeType(
    ...
    const toTimeType = () =>
      f.createTypeReferenceNode(f.createIdentifier(isTimeTz ? 'ClockTimeTz' : 'ClockTime'))

    // Handle union types (e.g., string | null)
    if (ts.isUnionTypeNode(typeNode)) {
      const updatedTypes = typeNode.types.map(t =>
        this.replaceStringWithClockTimeType(t, sourceFile, isTimeTz)
      )
   ...
   */
  private removeIgnoredColumns(dbSourceFile: ts.SourceFile) {
    const dbInterface = this.findDbExport(dbSourceFile, 'DB') as ts.InterfaceDeclaration | undefined
    if (!dbInterface) return dbSourceFile
    ...
    const ignoredColumnsByInterfaceName: Record<string, Set<string>> = {}

    dbInterface.members.forEach(member => {
      if (!ts.isPropertySignature(member)) return
      if (!member.type || !ts.isTypeReferenceNode(member.type)) return
      if (!ts.isIdentifier(member.type.typeName)) return
      ...
      if (ignoredColumns.size) ignoredColumnsByInterfaceName[member.type.typeName.text] = ignoredColumns
    })

    if (!Object.keys(ignoredColumnsByInterfaceName).length) return dbSourceFile
