...
${output}`
  ...
  private coercedType(
    ...
    const isTime = /^time without time zone[[\]]*$/.test(dbType)
    const isTimeTz = /^time with time zone[[\]]*$/.test(dbType)
    ...
    const types = compact(
      extractedPropertyTypes.map(typeNode => {
        ...
        switch (sanitized) {
          ...
          case 'Timestamp':
            if (isTime) return f.createTypeReferenceNode('TimeWithoutZone')
            if (isTimeTz) return f.createTypeReferenceNode('TimeWithZone')
            return isDate ? f.createTypeReferenceNode('CalendarDate') : f.createTypeReferenceNode('DateTime')

          case 'Timestamp[]':
            if (isTime) return f.createArrayTypeNode(f.createTypeReferenceNode('TimeWithoutZone'))
            if (isTimeTz) return f.createArrayTypeNode(f.createTypeReferenceNode('TimeWithZone'))
            return isDate
