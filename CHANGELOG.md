...
- `g:migration --help` now describes how `-to-<table_name>` and `-from-<table_name>` are actually matched. The help text called them suffixes and told you to "End with" one, but the FIRST `-to-` or `-from-` **anywhere** in the migration name selects `alterTable` mode, and everything after it is read as the table name. So `create-many-to-many-posts-users` matches the `-to-` inside `many-to-many` and resolves the table to `many_posts_users` — which is why the help now carries that name as an explicit counter-example, paired on the next line with a rename (`create-posts-users-join-table`) that avoids the marker. The help also no longer claims that a name matching no marker produces an "empty migration" with "empty up/down methods": it scaffolds `alterTable('<table-name>')` with the placeholder table for you to hand-edit, which is what it has always done. Documentation only — the matching behavior is unchanged.

- The error raised when an auto-selected `alterTable` migration ends up with no usable columns now explains why alter mode was selected at all. It previously named only the resolved table — a table you never typed, and which need not exist — and prescribed "pass at least one column declaration", which is the wrong remedy when the name merely happens to contain `-to-` and you meant to hand-write the migration (`renameTable`, `addEnumValue`, an index). The message now quotes the migration name and the marker that matched within it, states that the first marker anywhere in the name wins, and offers both remedies: pass a column declaration with a resolvable type, or rename the migration so that neither `-to-` nor `-from-` appears anywhere in it. Only the message text changed; the same names still error, and still write no file.
