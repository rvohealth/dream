...
- `psy i:serialization` (`inspect:serialization`), handed an **STI base**, now displays the union of every STI child's serializer, matching what `preloadFor` loads. It previously printed the base's name followed by a single child's serializer — the alphabetically-first one — as though that serializer were the base's, and walked only it, so an association rendered only by a later-sorting child was missing from the displayed tree. On this branch's own fixtures, `psy i:serialization Balloon stiUnion` omitted both `heartRatings` and `balloonLine`, the two associations the same release taught `preloadFor` to load — so the command contradicted the thing it exists to verify. The header now lists every serializer resolved for the key, one per line, and the tree beneath it is the union. An association several children render identically prints once; a polymorphic association still prints one line per target class (`Polymorphic/Task#taskable` prints a line for `Chore` and a line for `Workout`), and an association whose target is itself an STI base still prints one line per child serializer. Delegated attributes remain invisible to this command, as before. Handing the command an STI child, or any non-STI model, prints exactly what it printed before.
