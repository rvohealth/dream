...
- `SingleDbCredential` now accepts optional pg pool/client timeout passthrough: `connectionTimeoutMillis`, `idleTimeoutMillis`, `maxLifetimeSeconds`, `max`, `statement_timeout`, `query_timeout`. node-postgres ships unprotective defaults (`connectionTimeoutMillis: 0` = wait forever on an exhausted pool; no `statement_timeout`/`query_timeout`), which is the _prevention_ gap behind the shutdown hang above — a query that can't time out can leak a connection. Setting `connectionTimeoutMillis` / `statement_timeout` lets a stuck query self-abort and release its client, so the bounded drain above is rarely even reached. **Backward compatible:** every field is optional and only forwarded to `pg.Pool` when explicitly set, so an existing credential is byte-for-byte identical to pre-2.11.2 (pg keeps applying its own defaults). No timeout is defaulted at the library layer (a non-zero default would change behavior for every existing app, and `statement_timeout`/`query_timeout` defaults would kill legitimate long migrations/reports — set `statement_timeout` on the app's Postgres role instead). `@rvoh/create-psychic` ships recommended values in the scaffolded `conf/dream.ts` for new apps.
