...
describe('Dream.toKysely', () => {
  ...
  context('select', () => {
    it('returns a valid kysely query with correct types', () => {
      const kyselyQuery = User.toKysely('select')
    ...
    context('with a joined association', () => {
      it('returns a valid kysely query with correct types', () => {
        const kyselyQuery = User.leftJoin('pets').toKysely('select')
        expect(kyselyQuery.compile().sql).toEqual(
      ...
      context('with an aliased association', () => {
        it('returns a valid kysely query with correct types', () => {
          const kyselyQuery = User.leftJoin('pets as p').toKysely('select')
          expect(kyselyQuery.compile().sql).toEqual(
  ...
  context('update', () => {
    it('returns a valid kysely query with correct types', () => {
      const kyselyQuery = User.toKysely('update').set('email', 'how@yadoin')
      expect(kyselyQuery.compile().sql).toEqual(
  ...
  context('delete', () => {
    it('returns a valid kysely query with correct types', () => {
      const kyselyQuery = User.toKysely('delete')
    ...
    context('with a joined association', () => {
      it('returns a valid kysely query with correct types', () => {
        const kyselyQuery = User.leftJoin('pets').toKysely('delete')
        expect(kyselyQuery.compile().sql).toEqual(
      ...
      context('with an aliased association', () => {
        it('returns a valid kysely query with correct types', () => {
          const kyselyQuery = User.leftJoin('pets as p').toKysely('delete')
          expect(kyselyQuery.compile().sql).toEqual(
