...
describe('Dream Scope (default variant)', () => {
  ...
  it('exposes a method which auto-applies scope', async () => {
    const user1 = await User.create({ email: 'how@ya0', password: 'doin', deletedAt: DateTime.now() })
    const user2 = await User.create({ email: 'how@ya1', password: 'doin', deletedAt: DateTime.now() })
