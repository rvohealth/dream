...
describe('Dream.avgBy', () => {
  ...
  it('groups the average of the aggregated column across all records of a model', async () => {
    ...
    const result = await CompositionAsset.avgBy('name', 'score')
  ...
  context('when passed a transaction', () => {
    it('reports accurate grouped averages (builder path)', async () => {
      await CompositionAsset.create({ composition, name: 'primary', score: 3 })
      ...
      await ApplicationModel.transaction(async txn => {
        ...
        const result = await CompositionAsset.txn(txn).avgBy('name', 'score')
        expect(result).toEqual(
          new Map<string | null, number | null>([
            ['primary', 5],
            ['secondary', 4],
