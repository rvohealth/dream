...
describe('Dream#includes', () => {
  ...
  context('polymorphic', () => {
    it('loads a HasMany association', async () => {
      ...
      const rating = await Rating.create({ rateable_id: post.id, rateable_type: 'Rating' })

      const reloaded = await Post.includes('ratings').first()
    ...
    it('loads a BelongsTo association', async () => {
      ...
      const composition = await Composition.create({ user_id: user.id })
      const post = await Post.create({ user_id: user.id })
      const rating = await Rating.create({ rateable_id: post.id, rateable_type: 'Post' })

      const reloaded = await Rating.includes('rateable').first()
