...
describe('Dream.pluckEachThrough', () => {
  it('can pluck from the associated namespace', async () => {
    ...
    const plucked: any[] = []
    await Node.pluckEachThrough('edgeNodes', 'edge', { name: 'E1' }, ['edge.id', 'edge.name'], data => {
      plucked.push(data)
    })
  ...
  it('association name after conditional', async () => {
    ...
    const plucked: any[] = []
    await Node.pluckEachThrough('edgeNodes', { edgeId: edge2.id }, 'edge', ['edge.id', 'edge.name'], data => {
      plucked.push(data)
    })
...
describe('Dream#pluckEachThrough', () => {
  ...
  context('with a transaction', () => {
    it('supports pluckEachThrough', async () => {
      const node = await Node.create({ name: 'N1' })
      const edge1 = await Edge.create({ name: 'E1' })
      const edge2 = await Edge.create({ name: 'E2' })
      await EdgeNode.create({ node, edge: edge1 })
      ...
      await Node.transaction(async txn => {
        await node
          .txn(txn)
          .pluckEachThrough('edgeNodes', 'edge', { name: 'E1' }, ['edge.id', 'edge.name'], data => {
            plucked.push(data)
          })
      })
