...
describe('Dream.maxThrough', () => {
  it('returns the max field, first traveling through nested associations', async () => {
    ...
    const max = await composition.maxThrough('compositionAssets', 'compositionAssets.score')
  ...
  context('when passed a where clause', () => {
    it('respects the where clause', async () => {
      ...
      const max = await composition.maxThrough(
        'compositionAssets',
        { name: 'howyadoin' },
  ...
  context('when passed a transaction', () => {
    it('returns the max, traveling through nested associations', async () => {
      ...
      await ApplicationModel.transaction(async txn => {
        ...
        max = await user.txn(txn).maxThrough('compositions', 'compositionAssets', 'compositionAssets.score')
      })
