...
describe('Dream.sumBy', () => {
  ...
  it('groups the sum of the aggregated column across all records of a model', async () => {
    ...
    const result = await CompositionAsset.sumBy('name', 'score')
  ...
  context('when passed a transaction', () => {
    it('reports accurate grouped sums (builder path)', async () => {
      await CompositionAsset.create({ composition, name: 'primary', score: 3 })
      ...
      await ApplicationModel.transaction(async txn => {
        ...
        const result = await CompositionAsset.txn(txn).sumBy('name', 'score')
        expect(result).toEqual(
          new Map<string | null, number | null>([
            ['primary', 10],
            ['secondary', 4],
