...
describe('Dream.maxBy', () => {
  ...
  it('groups the max of the aggregated column across all records of a model', async () => {
    ...
    const result = await CompositionAsset.maxBy('name', 'score')
  ...
  context('when passed a transaction', () => {
    it('reports accurate grouped maxes (builder path)', async () => {
      await CompositionAsset.create({ composition, name: 'primary', score: 3 })
      ...
      await ApplicationModel.transaction(async txn => {
        ...
        const result = await CompositionAsset.txn(txn).maxBy('name', 'score')
        expect(result).toEqual(
          new Map<string | null, number | null>([
            ['primary', 7],
            ['secondary', 4],
