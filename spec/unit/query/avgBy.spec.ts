...
describe('Query#avgBy', () => {
  ...
  it('groups the average of the aggregated column by the group column', async () => {
    ...
    const result = await CompositionAsset.query().avgBy('name', 'score')
  ...
  context('with a where clause', () => {
    it('respects the where clause', async () => {
      ...
      const result = await CompositionAsset.where({ name: 'primary' }).avgBy('name', 'score')
      expect(result).toEqual(new Map<string | null, number | null>([['primary', 5]]))
    })
  })

  context('with no matching records', () => {
    it('returns an empty Map', async () => {
      const result = await CompositionAsset.where({ name: 'nonexistent-name' }).avgBy('name', 'score')
      expect(result).toEqual(new Map())
    })
  })

  context('within an association query', () => {
    it('groups the average for only the association records', async () => {
      await CompositionAsset.create({ composition, name: 'primary', score: 3 })
      await CompositionAsset.create({ composition, name: 'primary', score: 7 })
      await CompositionAsset.create({ composition, name: 'secondary', score: 4 })
      ...
      const result = await composition.associationQuery('compositionAssets').avgBy('name', 'score')
      expect(result).toEqual(
        new Map<string | null, number | null>([
          ['primary', 5],
          ['secondary', 4],
  ...
  context('on a join', () => {
    it('groups the average by a joined association column', async () => {
      await CompositionAsset.create({ composition, name: 'primary', score: 3 })
      await CompositionAsset.create({ composition, name: 'primary', score: 7 })
      await CompositionAsset.create({ composition, name: 'secondary', score: 4 })
      ...
      const result = await Composition.query()
        ...
        .avgBy('compositionAssets.name', 'compositionAssets.score')
      expect(result).toEqual(
        new Map<string | null, number | null>([
          ['primary', 5],
          ['secondary', 4],
    ...
    context('when passed a transaction', () => {
      it('reports accurate grouped averages (Query#txn path)', async () => {
        await CompositionAsset.create({ composition, name: 'primary', score: 3 })
        ...
        await ApplicationModel.transaction(async txn => {
          ...
          const result = await CompositionAsset.query().txn(txn).avgBy('name', 'score')
          expect(result).toEqual(
            new Map<string | null, number | null>([
              ['primary', 5],
              ['secondary', 4],
