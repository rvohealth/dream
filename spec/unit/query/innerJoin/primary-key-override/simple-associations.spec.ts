  ...
  context('sibling joins', () => {
    it('loads specified associations', async () => {
      ...
      const reloadedUsers = await User.query()
        .innerJoin('petsFromUuid', { and: { name: 'Violet' } })
        .innerJoin('petsFromUuid', 'collars', { and: { tagName: 'Violet' } })
        .all()
      expect(reloadedUsers).toMatchDreamModels([user])

      const noResults1 = await User.query()
        .innerJoin('petsFromUuid', { and: { name: 'Aster' } })
        .innerJoin('petsFromUuid', 'collars', { and: { tagName: 'Aster' } })
        .all()
