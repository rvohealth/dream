...
describe('ViewModelSerializer attributes', () => {
  it('can render Dream attributes', () => {
    const MySerializer = (data: User) =>
      ViewModelSerializer(User, data).attribute('email', { openapi: 'string' })
  ...
  it('supports customizing the name of the thing rendered', () => {
    const MySerializer = (data: User) =>
      ViewModelSerializer(User, data).attribute('email', { as: 'email2', openapi: 'string' })
  ...
  it('can specify OpenAPI description', () => {
    const MySerializer = (data: User) =>
      ViewModelSerializer(User, data).attribute('email', {
  ...
  context('when serializing null', () => {
    it('renderedAttributes is null', () => {
      const MySerializer = (data: User | null) =>
        ViewModelSerializer(User, data).maybeNull().attribute('email', { openapi: 'string' })
  ...
  it('can render attributes from serializers that "extend" other serializers', () => {
    const BaseSerializer = (data: User) =>
      ViewModelSerializer(User, data).attribute('name', { openapi: ['string', 'null'] })
  ...
  context('numeric/decimal with precision', () => {
    it('rounds to specified precision', () => {
      const MySerializer = (data: ModelForOpenapiTypeSpecs) =>
        ViewModelSerializer(ModelForOpenapiTypeSpecs, data).attribute('volume', {
  ...
  context('with casing specified', () => {
    const MySerializer = (data: ModelForOpenapiTypeSpecs) =>
      ViewModelSerializer(ModelForOpenapiTypeSpecs, data).attribute('requiredNicknames', {
        openapi: 'string[]',
      })
