import sanitizeSerializedValue from '../../../src/helpers/sanitizeSerializedValue.js'

describe('sanitizeSerializedValue', () => {
  context('with a custom sanitization strategy', () => {
    it('uses the custom strategy', () => {
      expect(sanitizeSerializedValue('Hello', (val: string) => val.replace(/l/g, 'L'))).toBe('HeLLo')
    })
  })
  ...
  context('unicodeString strategy', () => {
    context('basic HTML characters', () => {
      it('converts less-than symbol to HTML entity', () => {
        expect(sanitizeSerializedValue('<', 'unicodeString')).toBe('\\u003c')
      })
    ...
    context('script injection attempts', () => {
      it('sanitizes script tags', () => {
        ...
        expect(sanitizeSerializedValue(input, 'unicodeString')).toBe(expected)
      })
    ...
    context('data preservation', () => {
      it('preserves exact original data when decoded', () => {
        ...
        const sanitized = sanitizeSerializedValue(originalCode, 'unicodeString')
      ...
      it('preserves code snippets without breaking functionality', () => {
        const jsCode = `function test() { return x < y && "string"; }`
        const sanitized = sanitizeSerializedValue(jsCode, 'unicodeString')
        ...
        // But preserves original when decoded
        expect(sanitized).toBe('function test() { return x \\u003c y \\u0026\\u0026 \\u0022string\\u0022; }')
      })
    })

    context('mixed content', () => {
      it('preserves safe characters while sanitizing dangerous ones', () => {
        const input = 'Hello <world> & "friends"!'
        const expected = 'Hello \\u003cworld\\u003e \\u0026 \\u0022friends\\u0022!'
        expect(sanitizeSerializedValue(input, 'unicodeString')).toBe(expected)
      })
    ...
    context('edge cases', () => {
      ...
      it('handles null and undefined', () => {
        expect(sanitizeSerializedValue(null, 'unicodeString')).toBeNull()
        expect(sanitizeSerializedValue(undefined, 'unicodeString')).toBeUndefined()
      })

      it('handles string with only safe characters', () => {
        const input = 'This is perfectly safe text with numbers 123 and symbols !@#$%^*()'
        expect(sanitizeSerializedValue(input, 'unicodeString')).toBe(input)
      })
    ...
    context('JSON serialization compatibility', () => {
      it('maintains HTML entities through JSON.stringify and JSON.parse', () => {
        ...
        const sanitized = sanitizeSerializedValue(input, 'unicodeString')
      ...
      it('works with complex objects that get JSONified', () => {
        ...
        const sanitized = {
          title: sanitizeSerializedValue(data.title, 'unicodeString'),
          content: sanitizeSerializedValue(data.content, 'unicodeString'),
          metadata: {
            author: sanitizeSerializedValue(data.metadata.author, 'unicodeString'),
          },
        }
    ...
    context('real-world attack vectors', () => {
      it('sanitizes data URIs', () => {
        ...
        const result = sanitizeSerializedValue(input, 'unicodeString')
    ...
    context('when used with template rendering', () => {
      it('produces safe output that displays correctly when rendered in HTML', () => {
        ...
        const sanitized = sanitizeSerializedValue(userInput, 'unicodeString')
    ...
    context('performance with large strings', () => {
      it('handles large strings efficiently', () => {
        ...
        const result = sanitizeSerializedValue(largeInput, 'unicodeString')
  ...
  context('htmlEntity strategy', () => {
    context('basic HTML characters', () => {
      it('converts less-than symbol to HTML entity', () => {
        expect(sanitizeSerializedValue('<', 'htmlEntity')).toBe('&lt;')
      })
    ...
    context('script injection attempts', () => {
      it('sanitizes script tags', () => {
        const input = '<script>alert("xss")</script>'
        const expected = '&lt;script&gt;alert(&quot;xss&quot;)&lt;&#x2F;script&gt;'
        expect(sanitizeSerializedValue(input, 'htmlEntity')).toBe(expected)
      })
    ...
    context('data preservation', () => {
      it('preserves exact original data when decoded', () => {
        const originalCode = 'if (x < 5 && y > "test") { console.log("hello"); }'
        const sanitized = sanitizeSerializedValue(originalCode, 'htmlEntity')
      ...
      it('preserves code snippets without breaking functionality', () => {
        const jsCode = `function test() { return x < y && "string"; }`
        const sanitized = sanitizeSerializedValue(jsCode, 'htmlEntity')
    ...
    context('edge cases', () => {
      ...
      it('handles null and undefined', () => {
        expect(sanitizeSerializedValue(null, 'htmlEntity')).toBeNull()
        expect(sanitizeSerializedValue(undefined, 'htmlEntity')).toBeUndefined()
      })

      it('handles string with only safe characters', () => {
        const input = 'This is perfectly safe text with numbers 123 and symbols !@#$%^*()'
        expect(sanitizeSerializedValue(input, 'htmlEntity')).toBe(input)
      })
    ...
    context('JSON serialization compatibility', () => {
      it('maintains HTML entities through JSON.stringify and JSON.parse', () => {
        const input = '<script>alert("test")</script>'
        const sanitized = sanitizeSerializedValue(input, 'htmlEntity')
        const jsonString = JSON.stringify({ content: sanitized })
        const parsed = JSON.parse(jsonString)
      ...
      it('works with complex objects that get JSONified', () => {
        ...
        const sanitized = {
          title: sanitizeSerializedValue(data.title, 'htmlEntity'),
          content: sanitizeSerializedValue(data.content, 'htmlEntity'),
          metadata: {
            author: sanitizeSerializedValue(data.metadata.author, 'htmlEntity'),
          },
        }
    ...
    context('real-world attack vectors', () => {
      it('sanitizes data URIs', () => {
        const input = '<img src="data:text/html,<script>alert(1)</script>">'
        const result = sanitizeSerializedValue(input, 'htmlEntity')
        expect(result).not.toContain('<script>')
        expect(result).toContain('&lt;script&gt;')
      })
    ...
    context('when used with template rendering', () => {
      it('produces safe output that displays correctly when rendered in HTML', () => {
        const userInput = 'User said: <script>alert("hack")</script>'
        const sanitized = sanitizeSerializedValue(userInput, 'htmlEntity')
    ...
    context('performance with large strings', () => {
      it('handles large strings efficiently', () => {
        const dangerousChar = '<script>alert("xss")</script>'
        const largeInput = dangerousChar.repeat(1000)
        const result = sanitizeSerializedValue(largeInput, 'htmlEntity')
